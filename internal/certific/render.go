...
func Render(baseDir string, certs []RenderedCert, keep int) (versionDir string, pruned []string, err error) {
	...
	if err := writeCerts(stagingDir, baseDir, certs); err != nil {
...
// writeCerts populates writeDir with one .crt and .key per cert plus a
...
//
// baseDir is `--out-dir` as Traefik sees it on its own filesystem —
// used to emit absolute paths in tls.yml. Relative paths there don't
// work: Traefik resolves them against its process CWD (`/` in the
// upstream image), not against the directory holding the dynamic
// config file, so a bare filename produces a misleading "unable to
// parse certificate" at startup. The paths point through the
// `current` symlink rather than the versioned dir so tls.yml's
// contents stay stable across version swaps — the symlink resolves
// to the right version at file-open time.
func writeCerts(writeDir, baseDir string, certs []RenderedCert) error {
	...
	for _, c := range certs {
		...
		certPath := filepath.Join(writeDir, slug+".crt")
		keyPath := filepath.Join(writeDir, slug+".key")
		...
		entries = append(entries, entry{
			cert: filepath.Join(currentDir, slug+".crt"),
			key:  filepath.Join(currentDir, slug+".key"),
		})
	}

	// Traefik dynamic config schema: a top-level `tls.certificates` list
	// where each entry points at certFile + keyFile. YAML is hand-emitted
	// to avoid pulling in a YAML dependency for ~20 lines of output.
	...
	if len(entries) == 0 {
		...
		return os.WriteFile(filepath.Join(writeDir, "tls.yml"), []byte(out), 0o600)
	}

	return os.WriteFile(filepath.Join(writeDir, "tls.yml"), []byte(b.String()), 0o600)
}
